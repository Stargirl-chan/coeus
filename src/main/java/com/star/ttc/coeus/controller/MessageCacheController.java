...
public class MessageCacheController {
	...
	@RequestMapping("/message-cache-old")
	public ModelAndView index() {
		logger.info(messageCacheService.findAll().toString());
		
		ModelAndView mav = new ModelAndView("message-cache-old");
		
		mav.addObject("items", messageCacheService.findAll());
		return mav;
	}
    ...
    public String indexPaginated(
      ...
      @RequestParam("size") Optional<Integer> size) {
        ...
        Page<MessageCache> messageCachePage = messageCacheService.findPaginated(PageRequest.of(currentPage - 1, pageSize));
